...
things_reply_keyboard = [["Конспекты"], ["ДЗ"], ["Выйти"]]
...
ACTION, ACTION_HW, SUBJECT, YEAR, MONTH, DAY, UPLOAD_PHOTOS, GET_PHOTOS, SEE_PHOTOS = range(9)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(entry_points=[CommandHandler("start", start)],
                                                states={ACTION: [MessageHandler(filters.Regex(choice_reg), choose_an_action)],
                                                        ACTION_HW:[MessageHandler(filters.Regex(choice_reg), choose_an_action_HW)],
...
async def start(update, context) -> int:
    ...
    if context.user_data["choice"] == "Конспекты":
        return ACTION
    else:
        return ACTION_HW
...
async def choose_an_action_HW(update, context) -> int:
    await update.message.reply_text("Что вы хотите сделать?", reply_markup=ReplyKeyboardMarkup(keyboard=[["Загрузить дз", "Посмтотреть, что заданно"], ["Выйти"]], one_time_keyboard=False, resize_keyboard=True))
    return SUBJECT
...
async def get_or_see_photos(update, context, bot) -> int:
    ...
    if context.user_data["choise"] == "Конспекты":
        ...
        if context.user_data["action"] == "Посмотреть конспекты":
            ...
            if not file_list:
                ...
                for i in file_list:
                    if i.file != None:
                        if i == "home_work.txt":
                            h_w = open(path + "home_work.txt", "r")
                            text = h_w.readlines()
                            update.message.reply_text(text)
                        else:
                            await bot.send_photo(update.message.chat.id, i.file)
    else:
        path = 'conspectbot/' + context.user_data["subject"] + '/' + context.user_data["year"] + '/' + context.user_data["month"] + '/' + context.user_data["day"] + '/' + "ДЗ"
        if context.user_data["action"] == "Загрузить дз":
            await update.message.reply_text("Отправте в чат изображения,или текст которые вы хотите добавить", reply_markup=ReplyKeyboardMarkup(keyboard=[["Выйти"]], one_time_keyboard=False, resize_keyboard=True))
            return UPLOAD_PHOTOS
        if context.user_data["action"] == "Посмотреть, что заданно":
            file_list = list(disk.listdir(path))
            if not file_list:
                await update.message.reply_text("Извините, на эту дату дз не выложенно, попросите выложить",reply_markup=ReplyKeyboardMarkup(keyboard=[["Выйти"]],one_time_keyboard=False, resize_keyboard=True))
            else:
                await update.message.reply_text(text="Вот всё дз, которое мне удалось найти:", reply_markup=ReplyKeyboardMarkup(keyboard=[["Выйти"]], one_time_keyboard=False, resize_keyboard=True))
                for i in file_list:
                    if i.file != None:
                        await bot.send_photo(update.message.chat.id, i.file)
...
async def upload_photos(update, context, bot) -> int:
    if context.user_data["choise"] == "Конспекты":
            path = 'conspectbot/' + context.user_data["subject"] + '/' + context.user_data["year"] + '/' + context.user_data["month"] + '/' + context.user_data["day"]
    elif context.user_data["choise"] == "ДЗ":
            path = 'conspectbot/' + context.user_data["subject"] + '/' + context.user_data["year"] + '/' + context.user_data["month"] + '/' + context.user_data["day"] + '/' + "ДЗ"
    if update.message.document:
            ...
            disk.upload_url(file.file_path, path)
    elif update.message.text and context.user_data["choise"] == "ДЗ":
            text = update.message.text
            text_file = open("home_work.txt", "w")
            text_file.write(text)
            disk.upload_url("home_work.txt", path)
            text_file.close()
