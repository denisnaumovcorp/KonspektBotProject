...
days_reg = "(^\d{1}$|^\d{2}$)"
action_reg = "(^Загрузить конспекты$|^Посмотреть конспекты$|^Загрузить ДЗ$|^Посмотреть, что задано$)"

things_reply_keyboard = [["Конспекты"], ["ДЗ"]]
action_reply_keyboard = [["Загрузить конспекты", "Посмотреть конспекты"], ["Назад"], ["Выйти"]]
action_reply_keyboard_HW = [["Загрузить ДЗ", "Посмотреть, что задано"], ["Назад"], ["Выйти"]]
...
async def choose_an_action(update, context) -> int:  # Функция выбора действия, задействующая action_reply_keyboard
    ...
    if context.user_data["choice"] == "ДЗ":
        await update.message.reply_text("Что вы хотите сделать?",
                                        reply_markup=ReplyKeyboardMarkup(
                                            keyboard=action_reply_keyboard_HW,
                                            one_time_keyboard=False,
                                            resize_keyboard=True))
    else:
        await update.message.reply_text("Что вы хотите сделать?",
...
async def choose_a_subject(update, context) -> int:  # Функция выбора учебного предмета
    ...
    if context.user_data["action"] == "Посмотреть конспекты" or context.user_data["action"] == "Посмотреть, что задано":
        ...
        await update.message.reply_text("Выберите предмет",
                                        reply_markup=ReplyKeyboardMarkup(
                                            ...
                                            one_time_keyboard=False,
                                            resize_keyboard=True))
    elif context.user_data["action"] == "Загрузить конспекты" or context.user_data["action"] == "Загрузить ДЗ":
...
async def get_or_see_photos(update, context):  # Функция просмотра или запуска функции загрузки фото и/или документов
    ...
    # Ветвление, определяющие, какое действие выбрал пользователь
    if context.user_data["action"] == "Загрузить конспекты" or context.user_data["action"] == "Загрузить ДЗ":
    ...
    # Ветвление, определяющие, какое действие выбрал пользователь
    elif context.user_data["action"] == "Посмотреть конспекты" or context.user_data["action"] == "Посмотреть, что задано":
        ...
        if existing_files is empty:
            ...
            await update.message.reply_text("Извините, на эту дату конспекты/дз не выложены, попросите выложить",
                                            reply_markup=ReplyKeyboardMarkup(keyboard=[["Назад"], ["Выйти"]],
                                                                             one_time_keyboard=False,
                                                                             resize_keyboard=True))
            ...
            if yandex_disk.is_file(context.user_data["path"]):
                h_w = open( context.user_data["path"] + "home_work.txt", "r")
                text = h_w.readlines()
...
async def upload_photos(update, context) -> None:  # Функция для загрузки фото и/или документов
    ...
    if update.message.document:
        ...
        temp_path += '/' + update.message.from_user["username"] + '_' + update.message.document.file_name
        ...
        yandex_disk.upload_url(file.file_path, temp_path)
    # elif context.user_data["choice"] == "ДЗ":
    #         text = update.message.text
    #         if not yandex_disk.is_file(temp_path + '/' + "home_work.txt"):
    #             text_file = open(temp_path + '/' + "home_work.txt", "w")
    #         else:
    #             text_file = open(temp_path + '/' + "home_work.txt", "a")
    #         text_file.write(text)
    #         yandex_disk.upload_url("home_work.txt", temp_path)
    #         text_file.close()
    # # elif update.message.text and context.user_data["choice"] != "ДЗ":
    # #     update.message.reply_text("Пришлите, пожалуйста фото или файл")
